...
let isInitialized = (global as any).isDatabaseInitialized || false;

export async function initDatabase() {
  if ((global as any).isDatabaseInitialized || isInitialized) return;
  try {
    ...
    isInitialized = true;
    (global as any).isDatabaseInitialized = true;
  } catch (err) {
...
export async function saveUserAssessment(
  ...
  try {
    ...
    await sql`
      ...
      VALUES (${profileId}, ${userId}, ${dbCategory}, ${wellnessLevel}, ${percentage}, ${totalScore})
    ...
    await sql`
      ...
      VALUES (${userId}, ${dbCategory}, ${stressFreq}, ${sleepQual}, ${supportLvl}, ${totalScore}, ${JSON.stringify(detailedAnswers)}::jsonb)
    `;
